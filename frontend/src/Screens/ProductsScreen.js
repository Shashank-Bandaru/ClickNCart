...
function ProductsScreen(props) {
  ...
  //const categorylist = ["Shirts","Pants","Wrist-watches","Books","Smartphones","Home Appliances","Wall Clocks"];
  ...
  const uploadFileHandler = (e) => {
    ...
    setUploading(true);
    axios
      .post(`${process.env.REACT_APP_LINK}/api/uploads`, bodyFormData, {
        headers: {
  ...
  return <div className="content content-margined">
    ...
    <div className="product-header">
      <h1 style={{fontFamily:"sans-serif",fontWeight:"bold"}}>Products</h1>
      ...
      <table className="table">
        ...
        <tbody>
          {products.map(product => (<tr key={product._id}>
            ...
            <td>{product.brand}</td>
            <td>
              <button className="button mx-3 bg-primary text-white" onClick={() => openModal(product)} >Edit</button>
              {' '}
              <button className="button bg-danger text-white" onClick={() => deleteHandler(product)} >Delete</button>
            </td>
          </tr>))}
        </tbody>
      </table>
